...
namespace Film_Finder
{
    public partial class Form1 : Form
    {
                        ...
                        if (xnchild.NodeType == XmlNodeType.Element)
                        {
                            switch (xnchild.Name)
                            {
                                case "title":
                                    ...
                                    break;
                                case "year":
                                    nMovie.Year = xnchild.InnerText.Trim();
                                    break;
                                case "actor":
                                    ...
                                    break;
                                case "certification":
                                    nMovie.Certification = xnchild.InnerText.Trim();
                                    break;
                                case "rating":
                                    nMovie.Rating = xnchild.InnerText.Trim();
                                    break;
                                case "length":
                                    nMovie.Time = xnchild.InnerText;
                                    break;
                                case "genre":
                                    genres.Add(xnchild.InnerText.Trim());
                                    break;
                                case "director":
        ...
        private void movieGraph_Paint(object sender, PaintEventArgs e)
        {
            ...
            float upY = height/20;
            float downY = height;
            float frontX = width / 20;
            float backX = width;

            //scale
            float scaleX = backX / 23;
            float scaleY = downY / 10;
            ...
            using (Font myFont = new Font("Arial", scaleY / 9 < 11 ? scaleY / 9 : 11))
            {
                //Year label (horizontal)
                float tempX = frontX - 8;
                ...
                float tempY = (downY + upY) - 8;
                for (float i = minRating; i <= maxRating; i++)
                {
            ...
            foreach (Movie movie in Movies)
            {
                if (filter(movie))
                {
                        ...
                        && movie.Irate >= minRating && movie.Irate <= maxRating)
                    {
                        ...
                        tempPoint = getCordinate((movie.Iyear - minYear), (movie.Irate - minRating), frontX, (downY + upY)-6, scaleX, scaleY);
                        movie.graphPoint = tempPoint;

                            g.FillEllipse(tempBrush, tempPoint.X, tempPoint.Y, scaleY / 9, scaleY / 9);
        ...
        public PointF getCordinate(float year, float rating, float x, float y, float scaleX, float scaleY)
        {

            return new PointF(x + (year * (scaleX)) , y - (rating * scaleY));
        }
        ...
        private void movieGraph_MouseClick(object sender, MouseEventArgs e)
        {
             ...
             float height = (movieGraph.Height * 9) / 10;
             float width = (movieGraph.Width * 8) / 10;
            ...
            while (x < Movies.Count && !found)
            {
                ...
                if (loc.X + radius >= temp.X && loc.X - radius < temp.X 
                    && loc.Y + radius >= temp.Y && loc.Y - radius < temp.Y )
                {
                    found = true;               
                     tempLine = new Point((int)(temp.X + 15), (int)((MapPreview.Height + temp.Y > movieGraph.Height)?
                        temp.Y - MapPreview.Height :(temp.Y + 15)));
                    MapPreview.Location = tempLine;
        ...
        private void selectAll_CheckedChanged(object sender, EventArgs e)
        {
            if (selectAll.Checked)
            {
                ...
                PG.CheckState = CheckState.Unchecked;
                PG13.CheckState = CheckState.Unchecked;
                R.CheckState = CheckState.Unchecked;
                Approved.CheckState = CheckState.Unchecked;
                NC17.CheckState = CheckState.Unchecked;
                GP.CheckState = CheckState.Unchecked;
                Unrated.CheckState = CheckState.Unchecked;
                Passed.CheckState = CheckState.Unchecked;
                Not.CheckState = CheckState.Unchecked;
                X.CheckState = CheckState.Unchecked;
                TVG.CheckState = CheckState.Unchecked;
                TVMA.CheckState = CheckState.Unchecked;
                TV14.CheckState = CheckState.Unchecked;
                NLA.CheckState = CheckState.Unchecked;
            }
            movieGraph.Refresh();
        }
        ...
        private void Clear_Click(object sender, EventArgs e)
        {
            ...
            trackBar1.Value = 350;
            range = trackBar1.Value;
            LenghtValue.Text = trackBar1.Value + "mins";
